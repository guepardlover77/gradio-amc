...
def capture_db(project_dir):
    ...
    # Box 1: Q1 réponse A — cochée (black/total > 0.15)
    cur.execute(
        "INSERT INTO capture_zone (student, page, copy, type, id_a, id_b, "
        "total, black, manual) VALUES (1, 1, 0, 4, 1, 1, 1000, 300, -1)"
    )
    z1 = cur.lastrowid
    # Box 2: Q1 réponse B — non cochée
    cur.execute(
        "INSERT INTO capture_zone (student, page, copy, type, id_a, id_b, "
        "total, black, manual) VALUES (1, 1, 0, 4, 1, 2, 1000, 50, -1)"
    )
    z2 = cur.lastrowid
    # Box 3: Q2 réponse A — forcé coché manuellement
    cur.execute(
        "INSERT INTO capture_zone (student, page, copy, type, id_a, id_b, "
        "total, black, manual) VALUES (1, 1, 0, 4, 2, 1, 1000, 50, 1)"
    )
    z3 = cur.lastrowid
    # Box 4: Q2 réponse B — forcé non-coché manuellement
    cur.execute(
        "INSERT INTO capture_zone (student, page, copy, type, id_a, id_b, "
        "total, black, manual) VALUES (1, 1, 0, 4, 2, 2, 1000, 400, 0)"
    )
    z4 = cur.lastrowid
    ...
    positions = [
        ...
        (z1, 1, 4, 100.0, 100.0), (z1, 3, 4, 130.0, 130.0),
        (z2, 1, 4, 200.0, 100.0), (z2, 3, 4, 230.0, 130.0),
        (z3, 1, 4, 100.0, 200.0), (z3, 3, 4, 130.0, 230.0),
        (z4, 1, 4, 200.0, 200.0), (z4, 3, 4, 230.0, 230.0),
    ]
    cur.executemany(
